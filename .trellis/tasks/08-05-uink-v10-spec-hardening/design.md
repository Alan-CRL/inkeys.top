...
本任务只完善公开规范、VuePress 导航和可下载样例。任务实施期间 version `10` 仍允许不兼容修改；完成规范加固和可读性审校后，当前文档冻结为 UInk 1.0 Beta 的首个兼容基线。

## Beta 冻结决定

- `Header.version` 继续使用 `10`，不因发布状态变化而改号。
- Header 布局、已注册 Type ID、字段类型与既有字段语义、对象作用域、排序规则以及保存恢复契约不再允许不兼容修改。
- 不改变既有对象解释的澄清和可选扩展可以继续使用 version `10`；其他不兼容修改必须使用新的 `Header.version`。
- 旧预 Beta 草案仍不属于兼容范围，不新增迁移要求。
