...
`contentId` 只标识当前文件版本中当前 Canvas 的物理顺序。完整重写会重新整理编号，其他文件、外部元数据或后续版本不得把它当作稳定对象身份。
...
- Erase 保存完整擦除形状，并按记录顺序作用于同一 Canvas 中此前的 Ink 与 Shape，不作用于 Media。具体裁剪、透明和背景效果由软件决定。
- Pen 使用块级颜色，并忽略 `opacity`，以完全不透明方式绘制。
- Highlighter 使用块级 `color` 和 `opacity`。读取器先生成整条笔迹的一次覆盖，再用 alpha source-over 合成，不能让采样段之间的重叠重复增加透明度。
- Advanced Highlighter 使用相同的整条覆盖与 source-over 规则，但可以由部分点提供颜色与透明度锚点；没有任何点级样式时回退块级样式。
...
Ink 和 Shape 共享 `renderOnlyWhenLatest` 规则。读取器忽略 Media，从当前 Canvas 尾部反向检查 Ink/Shape：末尾连续且标记为 `true` 的 Ink/Shape 构成最新组并正常显示；其他标记内容隐藏。遇到第一个未标记的 Ink/Shape 后，最新组结束。

该机制适用于形状修正：软件先保存多条原始 Ink/Shape 并设置标记，再保存未标记的结果 Shape。结果存在时原稿隐藏；撤回结果并完整重写后，原稿成为末尾标记组并重新显示。

latest 分组只影响显示，不合并或修改 `undoId`。撤回结果后，用户仍按原稿各自的 undoId 逐步撤回。隐藏但未撤回的原稿是当前有效内容，完整保存必须保留；UInk 不承诺在保存并关闭后保留 Redo 历史。

## 擦除模型

### 擦除墨迹

`inkType = 0` 保存完整的擦除轨迹，可以像其他 Ink 一样在抬笔后追加。它不会要求修改此前的内容块；读取器按物理顺序将其作用于同一 Canvas 中此前的 Ink 与 Shape，Media 始终不受影响。

### 普通板擦

软件也可以采用会修改或切断既有 Ink/Shape 的普通板擦。写入器必须把结果归一化为当前有效的完整对象并完整重写文件，不得通过追加制造旧对象与新片段并存的错误画面。多个结果片段可以共享同一个 `undoId`，实现一步撤回全部片段。
