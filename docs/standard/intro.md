...
- 墨迹拓展文件（`filename.uink.extra`）使用 ZIP 格式打包，保存由 [Media 块](blocks/media)引用的图片、SVG、音视频等资源。
...
资源路径必须是 ZIP 根目录内的安全相对路径。媒体的 MIME、几何、播放参数和顺序全部保存在 `.uink` 主文件中。
