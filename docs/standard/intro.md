...
::: warning 预 Beta 草案
当前规范版本号为 `10`（UInk 1.0 草案），尚未进入 Beta，也没有冻结线格式。当前文档覆盖此前所有 version `10` 草案，不承诺读取旧草案文件。
:::
...
两者相互对应，其中墨迹拓展文件是可选的。`.uink` 主文件始终是当前有效内容的权威来源；资源包缺失或资源不可用时，基础 Ink/Shape 仍必须正常加载。

#### 墨迹文件的定位

- 墨迹主文件用于快速加载、基础预览和内容交换。
- 墨迹拓展文件只是资源容器，不保存内容顺序、几何或撤回信息。
