...
.vp-swiper {
  margin: 24px 0;
  /* 建议：给一个相对定位基准，防止内部绝对定位溢出 */
  position: relative;
  /* 修复：如果是在 flip 模式下，设置背景透明或颜色，防止翻转时的视觉残留 */
  background-color: transparent;
}

.vp-swiper .swiper-slide {
  /* 强制变成 Flex 布局 */
  ...
  /* ★★★ 核心修复：添加背景色遮挡底下的图片 ★★★ */
  /* 使用 VuePress 的背景变量，如果没有则默认为白色 */
  background-color: var(--vp-c-bg, #fff) !important; 
  
  /* 确保 3D 变换时，背面的元素不可见（部分浏览器需要） */
...
/* 2. 处理带有链接的情况 */
/* 因为图片被 a 标签包裹，所以 a 标签本身也要变成居中容器，且占满 slide */
.vp-swiper .swiper-slide-link {
  display: flex !important;
  justify-content: center !important;
  align-items: center !important;
  width: 100% !important;
...
/* 链接内的图片鼠标样式 */
.vp-swiper .swiper-slide-link .swiper-slide-img {
